...
namespace glasssix {
    ...
    void config::set_pedestrian_min(const std::string& path) {
        std::ifstream lic_dir(path + "/pedestrian_min.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: pedestrian_min.json:directory =") + path + std::string("/pedestrian_min.json"));

        json temp;
        std::ifstream(path + "/pedestrian_min.json") >> temp;
        _pedestrian_min_config = temp.get<pedestrian_min_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "pedestrian_min")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:pedestrian_min:thread pool num is set to 1\n");
        }
    }
