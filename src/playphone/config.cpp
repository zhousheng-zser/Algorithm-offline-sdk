...
namespace glasssix {
    ...
    void config::set_playphone(const std::string& path) {
        std::ifstream lic_dir(path + "/playphone.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: playphone.json:directory =") + path + std::string("/playphone.json"));

        json temp;
        std::ifstream(path + "/playphone.json") >> temp;
        _playphone_config = temp.get<playphone_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "playphone")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:playphone:thread pool num is set to 1\n");
        }
    }
