...
#include "../../../include/g6/info_policeuniform.hpp"
...
namespace glasssix {
    struct policeuniform_new_param {
        GX_BEGIN_FIELDS(policeuniform_new_param);
    ...
    struct policeuniform_detect_param {
        ...
        GX_BEGIN_FIELDS(policeuniform_detect_param);
    ...
    struct policeuniform_detect_result {
        GX_BEGIN_FIELDS(policeuniform_detect_result);
        GX_FIELD(parser_result_status, status);
        GX_FIELD(policeuniform_info, detect_info);
        GX_END_FIELDS;
        GX_JSON_SERIALIZABLE(naming_convention::lower_case_with_underscores);
    };

    struct policeuniform : protocol_object {
        struct detect : parser_inout<policeuniform_detect_param, policeuniform_detect_result> {};
    };
} // namespace glasssix
