...
#include "../../../include/g6/info_pedestrian_labor.hpp"
...
namespace glasssix {
    struct pedestrian_labor_new_param {
        GX_BEGIN_FIELDS(pedestrian_labor_new_param);
    ...
    struct pedestrian_labor_detect_param {
        ...
        GX_BEGIN_FIELDS(pedestrian_labor_detect_param);
    ...
    struct pedestrian_labor_detect_result {
        GX_BEGIN_FIELDS(pedestrian_labor_detect_result);
        GX_FIELD(parser_result_status, status);
        GX_FIELD(pedestrian_labor_info, detect_info);
        GX_END_FIELDS;
        GX_JSON_SERIALIZABLE(naming_convention::lower_case_with_underscores);
    };

    struct pedestrian_labor : protocol_object {
        struct detect : parser_inout<pedestrian_labor_detect_param, pedestrian_labor_detect_result> {};
    };
} // namespace glasssix
