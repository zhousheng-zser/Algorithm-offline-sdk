...
namespace glasssix {
    ...
    public:
        ...
        algo_ptr() {
            try {
                ...
                nlohmann::json new_json(crossing_new_param{.device = _config->_crossing_config.device,
                    .models_directory            = _config->_configure_directory.models_directory});
                char* new_result_c = parser_create_instance("g6.crossing.detect_code", new_json.dump().c_str());
    ...
    gx_crossing_api::gx_crossing_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
        ...
        crossing_info safe_production_crossing(const gx_img_api& mat) {
            try {
                auto result_pool = pool->enqueue([&] {
                    ...
                    nlohmann::json execute_json(crossing_detect_param{
                        .algo_params =
                            crossing_detect_param::optional_params{.dyparams =
                                                                   crossing_detect_param::optional_params::dyparams_params{
                                                                       .conf_thres = _config->_crossing_config.conf_thres,
                                                                       .nms_thres  = _config->_crossing_config.nms_thres}},
                        .data_params =
                            crossing_detect_param::basic_params{.height = mat.get_rows(), .width = mat.get_cols()}});
                    ...
                    ans = std::move(json::parse(*execute_result.result).get<crossing_detect_info_result>().detect_info);
                    return ans;
                });
                return result_pool.get();
            } catch (const std::exception& ex) {
                bool flag = write_dump_img(mat, "_crossing_dump.jpg", _config->_configure_directory.dump_img_directory);
    ...
    private:
        ...
        std::string empower_algorithm_version = share_platform_name + "_" + share_empower_language + "_crossing_V3.0.0";
    ...
    crossing_info gx_crossing_api::safe_production_crossing(const gx_img_api& mat) {
        ...
        return impl_->safe_production_crossing(mat);
    }
