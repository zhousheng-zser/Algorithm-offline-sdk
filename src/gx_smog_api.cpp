...
namespace glasssix {
    ...
    thread_pool* pool_smog = nullptr;
    std::unordered_map<std::thread::id, algo_smog_ptr*> smog_thread_algo_ptr;
    ...
    public:
        void init() {
            if (pool_smog == nullptr)
                pool_smog = new thread_pool(_config->_configure_directory.thread_pool_num_smog);
#if (GX_EMPOWER_FLAG)  
    ...
    smog_info gx_smog_api::safe_production_smog(const gx_img_api& mat) {
        try {
            auto result_pool = pool_smog->enqueue([&] {
                std::thread::id id_ = std::this_thread::get_id();
                if (smog_thread_algo_ptr[id_] == nullptr) {
                    smog_thread_algo_ptr[id_] = new algo_smog_ptr();
                }
                auto ptr = smog_thread_algo_ptr[id_];
                smog_info ans;
