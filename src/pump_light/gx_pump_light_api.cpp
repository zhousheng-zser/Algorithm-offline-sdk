...
namespace glasssix {
    ...
    public:
        ...
        algo_ptr() {
            try {
                ...
                nlohmann::json new_json(pump_light_new_param{.device = _config->_pump_light_config.device,
                    .models_directory            = _config->_configure_directory.models_directory});
                char* new_result_c = parser_create_instance("g6.pump_light.detect_code", new_json.dump().c_str());
    ...
    gx_pump_light_api::gx_pump_light_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
            ...
            try {
                auto result_pool = pool->enqueue([&] {
                    ...
                    nlohmann::json execute_json(
                        pump_light_detect_param{.algo_params =
                                                    pump_light_detect_param::optional_params{
                                                        .dyparams =
                                                            pump_light_detect_param::optional_params::dyparams_params{
                                                                ...
                                                                .y4         = quadrangle[3].y}},
                        .data_params =
                            pump_light_detect_param::basic_params{.height = mat.get_rows(), .width = mat.get_cols()}
                    ...
                    ans = std::move(json::parse(*execute_result.result).get<pump_light_detect_info_result>().detect_info);
                    return ans;
                });
                return result_pool.get();
            } catch (const std::exception& ex) {
                bool flag = write_dump_img(mat, "_pump_light_dump.jpg", _config->_configure_directory.dump_img_directory);
    ...
    private:
        ...
        std::string empower_algorithm_version = share_platform_name + "_" + share_empower_language + "_pump_light_V3.0.0";
    ...
    pump_light_info gx_pump_light_api::safe_production_pump_light(
        ...
        return impl_->safe_production_pump_light(mat, quadrangle);
    }
