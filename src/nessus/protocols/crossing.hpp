...
#include "../../../include/g6/info_crossing.hpp"
...
namespace glasssix {
    struct crossing_new_param {
        GX_BEGIN_FIELDS(crossing_new_param);
    ...
    struct crossing_detect_param {
        ...
        GX_BEGIN_FIELDS(crossing_detect_param);
    ...
    struct crossing_detect_result {
        GX_BEGIN_FIELDS(crossing_detect_result);
        GX_FIELD(parser_result_status, status);
        GX_FIELD(crossing_info, detect_info);
        GX_END_FIELDS;
        GX_JSON_SERIALIZABLE(naming_convention::lower_case_with_underscores);
    };

    struct crossing : protocol_object {
        struct detect : parser_inout<crossing_detect_param, crossing_detect_result> {};
    };
} // namespace glasssix
