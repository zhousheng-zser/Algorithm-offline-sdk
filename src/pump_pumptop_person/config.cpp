...
namespace glasssix {
    ...
    void config::set_pump_pumptop_person(const std::string& path) {
        std::ifstream lic_dir(path + "/pump_pumptop_person.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: pump_pumptop_person.json:directory =") + path + std::string("/pump_pumptop_person.json"));

        json temp;
        std::ifstream(path + "/pump_pumptop_person.json") >> temp;
        _pump_pumptop_person_config = temp.get<pump_pumptop_person_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "pump_pumptop_person")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:pump_pumptop_person:thread pool num is set to 1\n");
        }
    }
