...
namespace glasssix {
    ...
    public:
        ...
        algo_ptr() {
            try {
                ...
                nlohmann::json new_json(sleep_new_param{.device = _config->_sleep_config.device,
                    .models_directory            = _config->_configure_directory.models_directory});
                char* new_result_c = parser_create_instance("g6.sleep.detect_code", new_json.dump().c_str());
    ...
    gx_sleep_api::gx_sleep_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
        ...
        sleep_info safe_production_sleep(const gx_img_api& mat) {
            try {
                auto result_pool = pool->enqueue([&] {
...
#endif
                    nlohmann::json execute_json(sleep_detect_param{
                        .algo_params =
                            sleep_detect_param::optional_params{
                                .dyparams =
                                    sleep_detect_param::optional_params::dyparams_params{
                                        .conf_thres        = _config->_sleep_config.conf_thres,
                                        .nms_thres         = _config->_sleep_config.nms_thres,
                                        .frame_count_thres = 1,
                                        .device_id         = -1}},
                        .data_params =
                            sleep_detect_param::basic_params{.height = mat.get_rows(), .width = mat.get_cols()}
                    ...
                    ans = std::move(json::parse(*execute_result.result).get<sleep_detect_info_result>().detect_info);
                    return ans;
                });
                return result_pool.get();
            } catch (const std::exception& ex) {
                bool flag = write_dump_img(mat, "_sleep_dump.jpg", _config->_configure_directory.dump_img_directory);
    ...
    sleep_info gx_sleep_api::safe_production_sleep(const gx_img_api& mat) {
        ...
        return impl_->safe_production_sleep(mat);
    }
    //  安全生产 烟雾检测
    sleep_info gx_sleep_api::safe_production_sleep(const gx_img_api& mat , int device_id) {
        if (impl_ == nullptr)
            throw std::runtime_error{"You need to create a new object !\n"};
        return impl_->safe_production_sleep(mat, device_id);
    }
