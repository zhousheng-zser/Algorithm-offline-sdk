...
namespace glasssix {
    ...
    void config::set_pump_weld(const std::string& path) {
        std::ifstream lic_dir(path + "/pump_weld.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: pump_weld.json:directory =") + path + std::string("/pump_weld.json"));

        json temp;
        std::ifstream(path + "/pump_weld.json") >> temp;
        _pump_weld_config = temp.get<pump_weld_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "pump_weld")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:pump_weld:thread pool num is set to 1\n");
        }
    }
