...
namespace glasssix {
    ...
    public:
        ...
        algo_ptr() {
            try {
                ...
                nlohmann::json new_json(workcloth_new_param{.device = _config->_workcloth_config.device,
                    .models_directory            = _config->_configure_directory.models_directory
                    });
                char* new_result_c = parser_create_instance("g6.workcloth.detect_code", new_json.dump().c_str());
    ...
    gx_workcloth_api::gx_workcloth_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
    ...
    public:
        ...
        workcloth_info safe_production_workcloth(
            ...
            if (color_hsv_list_id != 0 && color_hsv_list_id != 1)
                throw std::runtime_error{
                    "workcloth color_hsv_list_id error: color_hsv_list_id != 0 && color_hsv_list_id != 1 "};
            ...
            try {
                auto result_pool = pool->enqueue([&] {
                    ...
                    nlohmann::json execute_json(workcloth_detect_param{
                        .algo_params =
                            workcloth_detect_param::optional_params{
                                .dyparams =
                                    workcloth_detect_param::optional_params::dyparams_params{
                                        .posture_info_list = posture_list_temp,
                                        .conf_thres   = _config->_workcloth_config.conf_thres,
                                        .nms_thres   = _config->_workcloth_config.nms_thres,
                                        .color_hsv_cfg = _config->_workcloth_config.color_hsv_list[color_hsv_list_id]}},
                        .data_params =
                            workcloth_detect_param::basic_params{.height = mat.get_rows(), .width = mat.get_cols()}
                    });
                    ...
                    ans = std::move(json::parse(*execute_result.result).get<workcloth_detect_info_result>().detect_info);
                    return ans;
                });
                return result_pool.get();
            } catch (const std::exception& ex) {
                bool flag = write_dump_img(mat, "_workcloth_dump.jpg", _config->_configure_directory.dump_img_directory);
                throw std::runtime_error{
    ...
    private:
        ...
        std::string empower_algorithm_version = share_platform_name + "_" + share_empower_language + "_workcloth_V3.0.0";
    ...
    workcloth_info gx_workcloth_api::safe_production_workcloth(const gx_img_api& mat, int color_hsv_list_id) {
        ...
        return impl_->safe_production_workcloth(mat, color_hsv_list_id,posture_info_list);
    }
    //  安全生产 工服检测
