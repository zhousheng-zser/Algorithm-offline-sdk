...
#include "../../../include/g6/info_pump_glove.hpp"
...
namespace glasssix {
    struct pump_glove_new_param {
        GX_BEGIN_FIELDS(pump_glove_new_param);
    ...
    struct pump_glove_detect_param {

        struct confidence_params {
            GX_BEGIN_FIELDS(confidence_params);
            GX_FIELD(std::optional<float>, hand_conf_thres);
        ...
        GX_BEGIN_FIELDS(pump_glove_detect_param);
    ...
    struct pump_glove_detect_result {
        GX_BEGIN_FIELDS(pump_glove_detect_result);
        GX_FIELD(parser_result_status, status);
        GX_FIELD(pump_glove_info, detect_info);
        GX_END_FIELDS;
        GX_JSON_SERIALIZABLE(naming_convention::lower_case_with_underscores);
    };

    struct pump_glove : protocol_object {
        struct detect : parser_inout<pump_glove_detect_param, pump_glove_detect_result> {};
    };
} // namespace glasssix
