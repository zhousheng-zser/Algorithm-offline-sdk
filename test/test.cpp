    ...
    void thread_function_helmet() {
        ...
        for (int i = 0; i < T; ++i) {
            try {
                ...
                printf("with_helmet_list = %d without_helmet_list = %d\n", val.with_helmet_list.size(), val.without_helmet_list.size());
    ...
    void thread_function_refvest() {
        ...
        for (int i = 0; i < T; ++i) {
            try {
                ...
                printf("without_refvtst_list = %d with_refvtst_list = %d\n", val.without_refvtst_list.size() ,val.with_refvtst_list.size() );
    ...
    void thread_function_sleep() {
        ...
        for (int i = 0; i < T; ++i) {
            try {
                ...
                printf("desk_list = %d lying_list = %d standing_list = %d work_list = %d\n", val.desk_list.size(), val.lying_list.size(),
                    val.standing_list.size(), val.work_list.size());
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        //auto end      = std::chrono::high_resolution_clock::now();
        //auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
        //printf("refvest*1_helmet*2_flame*2_search_integration time = " << duration.count() << " microsecond" << std::endl;
