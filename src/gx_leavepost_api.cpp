...
namespace glasssix {
    ...
    thread_pool* pool_leavepost = nullptr;
    std::unordered_map<std::thread::id, algo_leavepost_ptr*> leavepost_thread_algo_ptr;
    ...
    public:
        void init() {
            if (pool_leavepost == nullptr)
                pool_leavepost = new thread_pool(_config->_configure_directory.thread_pool_num_leavepost);
#if (GX_EMPOWER_FLAG)  
    ...
    private:
        ...
        std::string empower_algorithm_version =
            share_platform_name + "_" + share_empower_language + "_LEAVEPOST_V1.0.2";
        std::vector<std::string> empower_algorithm_id_list = {"1"};
    ...
    leavepost_info gx_leavepost_api::safe_production_leavepost(const gx_img_api& mat) {
        try {
            auto result_pool = pool_leavepost->enqueue([&] {
                std::thread::id id_ = std::this_thread::get_id();
                if (leavepost_thread_algo_ptr[id_] == nullptr) {
                    leavepost_thread_algo_ptr[id_] = new algo_leavepost_ptr();
                }
                auto ptr = leavepost_thread_algo_ptr[id_];
                leavepost_info ans;
                ...
                auto result = ptr->protocol_ptr.invoke<leavepost::detect>(ptr->leavepost_handle,
                    leavepost_detect_param{.instance_guid = "",
                        ...
                        .params                           = leavepost_detect_param::confidence_params{.conf_thres =
                                                                                _config->_leavepost_config.conf_thres,
            ...
            bool flag = write_dump_img(mat, "_leavepost_dump.jpg");
