...
namespace glasssix {
    ...
    void config::set_refvest(const std::string& path) {
        std::ifstream lic_dir(path + "/refvest.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: refvest.json:directory =") + path + std::string("/refvest.json"));

        json temp;
        std::ifstream(path + "/refvest.json") >> temp;
        _refvest_config = temp.get<refvest_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "refvest")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:refvest:thread pool num is set to 1\n");
        }
    }
