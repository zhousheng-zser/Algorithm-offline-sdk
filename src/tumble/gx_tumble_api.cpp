...
namespace glasssix {
    ...
    public:
        ...
        algo_ptr() {
            try {
                ...
                nlohmann::json new_json(tumble_new_param{.device = _config->_tumble_config.device,
                    .models_directory            = _config->_configure_directory.models_directory});
                char* new_result_c = parser_create_instance("g6.tumble.detect_code", new_json.dump().c_str());
    ...
    gx_tumble_api::gx_tumble_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
        ...
        tumble_info safe_production_tumble(const gx_img_api& mat) {
            try {
                auto result_pool = pool->enqueue([&] {
                    ...
                    nlohmann::json execute_json(tumble_detect_param{
                        .algo_params =
                            tumble_detect_param::optional_params{.dyparams =
                                                                   tumble_detect_param::optional_params::dyparams_params{
                                                                       .conf_thres = _config->_tumble_config.conf_thres,
                                                                       .nms_thres  = _config->_tumble_config.nms_thres}},
                        .data_params =
                            tumble_detect_param::basic_params{.height = mat.get_rows(), .width = mat.get_cols()}
                    ...
                    ans = std::move(json::parse(execute_result.result).get<tumble_detect_info_result>().detect_info);
                    return ans;
                });
                return result_pool.get();
            } catch (const std::exception& ex) {
                bool flag = write_dump_img(mat, "_tumble_dump.jpg", _config->_configure_directory.dump_img_directory);
    ...
    tumble_info gx_tumble_api::safe_production_tumble(const gx_img_api& mat) {
        ...
        return impl_->safe_production_tumble(mat);
    }
