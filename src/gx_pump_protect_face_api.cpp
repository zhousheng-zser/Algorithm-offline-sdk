...
namespace glasssix {
    ...
    gx_pump_protect_face_api::gx_pump_protect_face_api() : impl_{std::make_unique<impl>()} {}
    gx_pump_protect_face_api::gx_pump_protect_face_api(const abi::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
    gx_pump_protect_face_api::~gx_pump_protect_face_api() {}
    gx_pump_protect_face_api::gx_pump_protect_face_api(gx_pump_protect_face_api&&) noexcept            = default;
    gx_pump_protect_face_api& gx_pump_protect_face_api::operator=(gx_pump_protect_face_api&&) noexcept = default;
    class gx_pump_protect_face_api::impl {
    public:
        void init() {
#if (GX_EMPOWER_FLAG)  
    ...
    private:
        ...
        std::vector<std::string> empower_algorithm_id_list = {"25", "37"};
    ...
    //  安全生产 防护面罩检测
    pump_protect_face_info gx_pump_protect_face_api::safe_production_pump_protect_face(const gx_img_api& mat) {
        try {
...
#endif
                auto result = ptr->protocol_ptr.invoke<pump_protect_face::detect>(ptr->pump_protect_face_handle,
                    pump_protect_face_detect_param{.instance_guid = "",
                        .format                       = _config->_pump_protect_face_config.format,
                        .height                       = mat.get_rows(),
                        .width                        = mat.get_cols(),
                        .params =
                            pump_protect_face_detect_param::confidence_params{
                            .conf_thres = _config->_pump_protect_face_config.conf_thres}},
                    str);
            ...
            bool flag = write_dump_img(mat, "_pump_protect_face_dump.jpg");
