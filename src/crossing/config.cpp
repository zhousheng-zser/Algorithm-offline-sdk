...
namespace glasssix {
    ...
    void config::set_crossing(const std::string& path) {
        std::ifstream lic_dir(path + "/crossing.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: crossing.json:directory =") + path + std::string("/crossing.json"));

        json temp;
        std::ifstream(path + "/crossing.json") >> temp;
        _crossing_config = temp.get<crossing_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "crossing")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:crossing:thread pool num is set to 1\n");
        }
    }
