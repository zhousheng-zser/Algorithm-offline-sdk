...
namespace glasssix {
    ...
    config::config() {
        ...
        set_climb_tumble_pedestrian(_path);
    ...
    void config::set_climb_tumble_pedestrian(const std::string& path) {
        std::ifstream lic_dir(path + "/climb_tumble_pedestrian.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: climb_tumble_pedestrian.json:directory =") + path + std::string("/climb_tumble_pedestrian.json"));

        json temp;
        std::ifstream(path + "/climb_tumble_pedestrian.json") >> temp;
        _climb_tumble_pedestrian_config = temp.get<climb_tumble_pedestrian_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "climb_tumble_pedestrian")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:climb_tumble_pedestrian:thread pool num is set to 1\n");
        }
    }
