...
namespace glasssix {
    ...
    void config::set_pedestrian(const std::string& path) {
        std::ifstream lic_dir(path + "/pedestrian.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: pedestrian.json:directory =") + path + std::string("/pedestrian.json"));

        json temp;
        std::ifstream(path + "/pedestrian.json") >> temp;
        _pedestrian_config = temp.get<pedestrian_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "pedestrian")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:pedestrian:thread pool num is set to 1\n");
        }
    }
