...
namespace {
    ...
    */
    ...
    std::string long_long_to_string(long long val, int width = -1) {
        ...
        switch (width) {
        case 1:
            sprintf(buf, "%01lld", val);
            return buf;
        case 2:
            sprintf(buf, "%02lld", val);
            return buf;
        case 3:
            sprintf(buf, "%03lld", val);
            return buf;
        case 4:
            sprintf(buf, "%04lld", val);
            return buf;
        case 5:
            sprintf(buf, "%05lld", val);
            return buf;
        case 6:
            sprintf(buf, "%06lld", val);
            return buf;
        case 7:
            sprintf(buf, "%07lld", val);
            return buf;
        case 8:
            sprintf(buf, "%08lld", val);
            return buf;
        case 9:
            sprintf(buf, "%09lld", val);
            return buf;
        default:
            break;
        }
    ...
    bool time_format_contains_ap(std::string_view format) {
        return std::find_if(format.cbegin(), format.cend(), [](char c) { return c == 'a' || c == 'A'; })
            != format.cend();
    }
} // namespace

enum class time_spec { utc, local };

/*
| Expression | Output                                                                |
| :--------: | :-------------------------------------------------------------------- |
|     yy     | The year as a two digit number (00 to 99)                             |
|    yyyy    | The year as a four digit number. If the year is negative, a minus sign|
|            | is prepended, making five characters.                                 |
...
|     z      | The fractional part of the second, to go after a decimal point,       |
|            | without trailing zeroes (0 to 999).                                   |
|    zzz     | The fractional part of the second, to millisecond precision, including|
|            | trailing zeroes where applicable (000 to 999).                        |
...
public:
    ...
    template <typename T>
    date_time(std::chrono::year_month_day date, std::chrono::hh_mm_ss<T> time, time_spec spec = time_spec::utc)
    ...
    std::string to_string(std::string_view format, time_spec spec = time_spec::local) const {
        ...
        while (i < format.size()) {
            ...
            case 'M':
                ...
                switch (repeat) {
                case 1:
                    result.append(long_long_to_string(month));
                    break;
                case 2:
                    result.append(long_long_to_string(month, 2));
                    break;
                }
                break;
            case 'd':
                ...
                switch (repeat) {
                case 1:
                    result.append(long_long_to_string(day));
                    break;
                case 2:
                    result.append(long_long_to_string(day, 2));
                    break;
                }
                break;
            case 'h':
                {
                    ...
                    switch (repeat) {
                    case 1:
                        result.append(long_long_to_string(hour));
                        break;
                    case 2:
                        result.append(long_long_to_string(hour, 2));
                        break;
                    }
                    break;
                }
            ...
            case 'm':
                ...
                switch (repeat) {
                case 1:
                    result.append(long_long_to_string(hms.minutes().count()));
                    break;
                case 2:
                    result.append(long_long_to_string(hms.minutes().count(), 2));
                    break;
                }
                break;
            case 's':
                ...
                switch (repeat) {
                case 1:
                    result.append(long_long_to_string(hms.seconds().count()));
                    break;
                case 2:
                    result.append(long_long_to_string(hms.seconds().count(), 2));
                    break;
                }
                break;
            case 'a':
