...
namespace glasssix {
    ...
    void config::set_helmet(const std::string& path) {
        std::ifstream lic_dir(path + "/helmet.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: helmet.json:directory =") + path + std::string("/helmet.json"));

        json temp;
        std::ifstream(path + "/helmet.json") >> temp;
        _helmet_config = temp.get<helmet_config>();
        helmet_is_load = true;
    }
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "helmet")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:helmet:thread pool num is set to 1\n");
        }
    }
