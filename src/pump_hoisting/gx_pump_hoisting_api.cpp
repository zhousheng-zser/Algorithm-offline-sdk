...
namespace glasssix {
    ...
    public:
        ...
        algo_ptr() {
            try {
                ...
                nlohmann::json new_json(pump_hoisting_new_param{.device = _config->_pump_hoisting_config.device,
                    .models_directory            = _config->_configure_directory.models_directory});
                char* new_result_c = parser_create_instance("g6.pump_hoisting.detect_code", new_json.dump().c_str());
    ...
    gx_pump_hoisting_api::gx_pump_hoisting_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
        ...
        pump_hoisting_info safe_production_pump_hoisting(const gx_img_api& mat, int device_id) {
            if (!camera_id) {
               camera_id = device_id;
                ...
                throw std::runtime_error{"(device_id:" + std::to_string(device_id)
                                                      + ") != (camera_id:" + std::to_string(camera_id) + ")\n"};
            ...
            try {
                auto result_pool = pool->enqueue([&] {
...
#endif
                    nlohmann::json execute_json(pump_hoisting_detect_param{
                        .algo_params =
                            pump_hoisting_detect_param::optional_params{
                                .dyparams =
                                    pump_hoisting_detect_param::optional_params::dyparams_params{
                                        ...
                                        .device_id      = device_id}},
                        .data_params = pump_hoisting_detect_param::basic_params{
                    ...
                    ans = std::move(json::parse(*execute_result.result).get<pump_hoisting_detect_info_result>().detect_info);
                    return ans;
                });
                return result_pool.get();
            } catch (const std::exception& ex) {
                bool flag = write_dump_img(mat, "_pump_hoisting_dump.jpg", _config->_configure_directory.dump_img_directory);
    ...
    private:
        ...
        std::string empower_algorithm_version = share_platform_name + "_" + share_empower_language + "_pump_hoisting_V3.0.0";
    ...
    pump_hoisting_info gx_pump_hoisting_api::safe_production_pump_hoisting(const gx_img_api& mat, int device_id) {
        ...
        return impl_->safe_production_pump_hoisting(mat, device_id);
    }
