...
GX_API(GXFACEAPIC) bool gx_user_remove_records(char* keys, bool is_mask = false);
GX_API(GXFACEAPIC) bool* gx_user_add_records(char* keys, char* mat_path, bool is_mask = false);
GX_API(GXFACEAPIC) bool* gx_user_update_records(char* keys, char* mat_path, bool is_mask = false);
GX_API(GXFACEAPIC) char* gx_detect_integration(char* mat_path, int top = 1, float min_similarity = 0.4, bool is_mask = false);
