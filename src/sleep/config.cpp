...
namespace glasssix {
    ...
    void config::set_sleep(const std::string& path) {
        std::ifstream lic_dir(path + "/sleep.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: sleep.json:directory =") + path + std::string("/sleep.json"));

        json temp;
        std::ifstream(path + "/sleep.json") >> temp;
        _sleep_config = temp.get<sleep_config>();
        sleep_is_load = true;
    }
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "sleep")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:sleep:thread pool num is set to 1\n");
        }
    }
