...
namespace glasssix {
    ...
    class GX_API(GXAPI) gx_wander_api {
    public:
        gx_wander_api();
        gx_wander_api(const std::string& config_path);
        ~gx_wander_api();
        ...
        //  安全生产 徘徊检测
        wander_info safe_production_wander(const gx_img_api& mat, long long current_time, int device_id,
            int interval, const std::vector<pedestrian_info::boxes>& person_list);

        //  安全生产 越界检测
        wander_limit_info safe_production_wander_limit(const gx_img_api& mat, long long current_time, int device_id,
