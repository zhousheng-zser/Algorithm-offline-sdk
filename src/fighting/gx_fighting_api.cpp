...
namespace glasssix {
    ...
    public:
        ...
        algo_ptr() {
            try {
                ...
                nlohmann::json new_json(fighting_new_param{.device = _config->_fighting_config.device,
                    .models_directory            = _config->_configure_directory.models_directory,
                    .batch = _config->_fighting_config.batch});
                char* new_result_c = parser_create_instance("g6.fighting.detect_code", new_json.dump().c_str());
    ...
    gx_fighting_api::gx_fighting_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
    ...
    public:
        ...
        ~impl() {}
        fighting_info safe_production_fighting(const std::vector<gx_img_api>& mat_list, const fighting_roi& roi) {
            try {
                auto result_pool = pool->enqueue([&] {
                    ...
                    if (_config->_fighting_config.batch != mat_list.size())
                        throw std::runtime_error{"Error: The config/fighting.json : batch != mat_list.size()"};
                    fighting_info ans;
                    ...
                    ans = std::move(json::parse(execute_result.result).get<fighting_detect_info_result>().detect_info);
                    return ans;
                });
    ...
    //  安全生产 打架检测
    fighting_info gx_fighting_api::safe_production_fighting(
        const std::vector<gx_img_api>& mat_list, const fighting_roi& roi) {
        if (impl_ == nullptr)
            throw std::runtime_error{"You need to create a new object !\n"};
        return impl_->safe_production_fighting(mat_list, roi);
    }
