...
namespace glasssix {
    ...
    void config::set_pump_hoisting(const std::string& path) {
        std::ifstream lic_dir(path + "/pump_hoisting.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: pump_hoisting.json:directory =") + path + std::string("/pump_hoisting.json"));

        json temp;
        std::ifstream(path + "/pump_hoisting.json") >> temp;
        _pump_hoisting_config = temp.get<pump_hoisting_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "pump_hoisting")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:pump_hoisting:thread pool num is set to 1\n");
        }
    }
