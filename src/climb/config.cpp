...
namespace glasssix {
    ...
    void config::set_climb(const std::string& path) {
        std::ifstream lic_dir(path + "/climb.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: climb.json:directory =") + path + std::string("/climb.json"));

        json temp;
        std::ifstream(path + "/climb.json") >> temp;
        _climb_config = temp.get<climb_config>();
        climb_is_load = true;
    }
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "climb")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:climb:thread pool num is set to 1\n");
        }
    }
