...
namespace glasssix {
    ...
    void config::set_pump_gate_status(const std::string& path) {
        std::ifstream lic_dir(path + "/pump_gate_status.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: pump_gate_status.json:directory =") + path + std::string("/pump_gate_status.json"));

        json temp;
        std::ifstream(path + "/pump_gate_status.json") >> temp;
        _pump_gate_status_config = temp.get<pump_gate_status_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "pump_gate_status")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:pump_gate_status:thread pool num is set to 1\n");
        }
    }
