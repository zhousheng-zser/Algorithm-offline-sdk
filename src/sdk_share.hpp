...
namespace glasssix {
    ...
    class climb_tumble_pedestrian_ptr {
    public:
        climb_tumble_pedestrian_ptr() {
            protocol_ptr.init(_config->_configure_directory.directory);
            for (int i = 0; i < _config->protocols_list["plugin_list"].size(); ++i) {
                std::string temp_str = _config->protocols_list["plugin_list"][i];
                if (temp_str == "climb_tumble_pedestrian") { 
                    try {
                        ...
                        climb_tumble_pedestrian_handle = protocol_ptr.make_instance<climb_tumble_pedestrian>(
                            climb_tumble_pedestrian_new_param{_config->_climb_tumble_pedestrian_config.device,
    ...
    public:
        ...
        void set_protocols_handl_pump_light() {
            ...
            pump_light_handle = protocol_ptr.make_instance<pump_light>(pump_light_new_param{_config->_pump_light_config.device,
                    _config->_pump_light_config.model_type, _config->_configure_directory.models_directory});
        }
