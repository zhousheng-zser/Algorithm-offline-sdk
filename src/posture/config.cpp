...
namespace glasssix {
    ...
    void config::set_posture(const std::string& path) {
        std::ifstream lic_dir(path + "/posture.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: posture.json:directory =") + path + std::string("/posture.json"));

        json temp;
        std::ifstream(path + "/posture.json") >> temp;
        _posture_config = temp.get<posture_config>();
        posture_is_load = true;
    }
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "posture")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:posture:thread pool num is set to 1\n");
        }
    }
