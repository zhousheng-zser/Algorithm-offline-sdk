...
namespace glasssix {
    ...
    void config::set_pump_light(const std::string& path) {
        std::ifstream lic_dir(path + "/pump_light.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: pump_light.json:directory =") + path + std::string("/pump_light.json"));

        json temp;
        std::ifstream(path + "/pump_light.json") >> temp;
        _pump_light_config = temp.get<pump_light_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "pump_light")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:pump_light:thread pool num is set to 1\n");
        }
    }
