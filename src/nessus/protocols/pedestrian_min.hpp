...
#include "../../../include/g6/info_pedestrian_min.hpp"
...
namespace glasssix {
    struct pedestrian_min_new_param {
        GX_BEGIN_FIELDS(pedestrian_min_new_param);
    ...
    struct pedestrian_min_detect_param {
        ...
        GX_BEGIN_FIELDS(pedestrian_min_detect_param);
    ...
    struct pedestrian_min_detect_result {
        GX_BEGIN_FIELDS(pedestrian_min_detect_result);
        GX_FIELD(parser_result_status, status);
        GX_FIELD(pedestrian_min_info, detect_info);
        GX_END_FIELDS;
        GX_JSON_SERIALIZABLE(naming_convention::lower_case_with_underscores);
    };

    struct pedestrian_min : protocol_object {
        struct detect : parser_inout<pedestrian_min_detect_param, pedestrian_min_detect_result> {};
    };
} // namespace glasssix
