...
namespace glasssix {
    ...
    void config::set_leavepost(const std::string& path) {
        std::ifstream lic_dir(path + "/leavepost.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: leavepost.json:directory =") + path + std::string("/leavepost.json"));

        json temp;
        std::ifstream(path + "/leavepost.json") >> temp;
        _leavepost_config = temp.get<leavepost_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "leavepost")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:leavepost:thread pool num is set to 1\n");
        }
    }
