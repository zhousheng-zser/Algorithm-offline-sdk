...
namespace glasssix {
    ...
    gx_vehicle_api::gx_vehicle_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
        ...
        vehicle_info safe_production_vehicle(const gx_img_api& mat) {
            try {
                auto result_pool = pool->enqueue([&] {
                    ...
                    nlohmann::json execute_json(vehicle_detect_param{
                        .algo_params =
                            vehicle_detect_param::optional_params{.dyparams =
                                                                   vehicle_detect_param::optional_params::dyparams_params{
                                                                       .conf_thres = _config->_vehicle_config.conf_thres,
                                                                       .nms_thres  = _config->_vehicle_config.nms_thres}},
                        .data_params =
                            vehicle_detect_param::basic_params{.height = mat.get_rows(), .width = mat.get_cols()}
                    ...
                    ans = std::move(json::parse(execute_result.result).get<vehicle_detect_info_result>().detect_info);
                    return ans;
                });
                return result_pool.get();
            } catch (const std::exception& ex) {
                bool flag = write_dump_img(mat, "_vehicle_dump.jpg", _config->_configure_directory.dump_img_directory);
    ...
    vehicle_info gx_vehicle_api::safe_production_vehicle(const gx_img_api& mat) {
        ...
        return impl_->safe_production_vehicle(mat);
    }
