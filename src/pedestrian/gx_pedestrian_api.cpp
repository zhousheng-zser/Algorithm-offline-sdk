...
namespace glasssix {
    ...
    public:
        ...
        algo_ptr() {
            try {
                ...
                nlohmann::json new_json(pedestrian_new_param{.device = _config->_pedestrian_config.device,
                    .models_directory            = _config->_configure_directory.models_directory});
                char* new_result_c = parser_create_instance("g6.pedestrian.classify_code", new_json.dump().c_str());
    ...
    gx_pedestrian_api::gx_pedestrian_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
        ...
        pedestrian_info safe_production_pedestrian(const gx_img_api& mat) {
            try {
                auto result_pool = pool->enqueue([&] {
                    ...
                    nlohmann::json execute_json(pedestrian_detect_param{
                        .algo_params =
                            pedestrian_detect_param::optional_params{.dyparams =
                                                                   pedestrian_detect_param::optional_params::dyparams_params{
                                                                       .conf_thres = _config->_pedestrian_config.conf_thres,
                                                                       .nms_thres  = _config->_pedestrian_config.nms_thres}},
                        .data_params =
                            pedestrian_detect_param::basic_params{.height = mat.get_rows(), .width = mat.get_cols()}
                    ...
                    ans =
                        std::move(json::parse(execute_result.result).get<pedestrian_detect_info_result>().detect_info);
                    return ans;
                });
                return result_pool.get();
            } catch (const std::exception& ex) {
                bool flag = write_dump_img(mat, "_pedestrian_dump.jpg", _config->_configure_directory.dump_img_directory);
    ...
    private:
        ...
        std::string empower_algorithm_version = share_platform_name + "_" + share_empower_language + "_pedestrian_V3.0.0";
    ...
    pedestrian_info gx_pedestrian_api::safe_production_pedestrian(const gx_img_api& mat) {
        ...
        return impl_->safe_production_pedestrian(mat);
    }
