...
#include "../../../include/g6/info_leavepost.hpp"
...
namespace glasssix {
    struct leavepost_new_param {
        GX_BEGIN_FIELDS(leavepost_new_param);
    ...
    struct leavepost_detect_param {
        ...
        GX_BEGIN_FIELDS(leavepost_detect_param);
    ...
    struct leavepost_detect_result {
        GX_BEGIN_FIELDS(leavepost_detect_result);
        GX_FIELD(parser_result_status, status);
        GX_FIELD(leavepost_info, detect_info);
        GX_END_FIELDS;
        GX_JSON_SERIALIZABLE(naming_convention::lower_case_with_underscores);
    };

    struct leavepost : protocol_object {
        struct detect : parser_inout<leavepost_detect_param, leavepost_detect_result> {};
    };
} // namespace glasssix
