...
namespace glasssix {
    ...
    void config::set_batterypilferers(const std::string& path) {
        std::ifstream lic_dir(path + "/batterypilferers.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: batterypilferers.json:directory =") + path + std::string("/batterypilferers.json"));

        json temp;
        std::ifstream(path + "/batterypilferers.json") >> temp;
        _batterypilferers_config = temp.get<batterypilferers_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "batterypilferers")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:batterypilferers:thread pool num is set to 1\n");
        }
    }
