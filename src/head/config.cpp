...
namespace glasssix {
    ...
    //void config::set_head(const std::string& path) {
    //    std::ifstream lic_dir(path + "/head.json");
    //    if (!lic_dir.is_open())
    //        throw std::runtime_error(std::string("Error: head.json:directory =") + path + std::string("/head.json"));

    //    json temp;
    //    std::ifstream(path + "/head.json") >> temp;
    //    _head_config = temp.get<head_config>();
    //    head_is_load = true;
    //}
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "head")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:head:thread pool num is set to 1\n");
        }
    }
