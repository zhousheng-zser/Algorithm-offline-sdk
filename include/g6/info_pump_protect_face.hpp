...
namespace glasssix {
    // 泵业护脸信息
    struct pump_protect_face_info {
        struct boxes {
            GX_BEGIN_FIELDS(boxes);
            GX_FIELD(float, score); // 置信度
            GX_FIELD(std::int32_t, category); // 1:未戴护目镜 0 : 待更新
        ...
        GX_BEGIN_FIELDS(pump_protect_face_info);

        GX_FIELD(abi::vector<boxes>, pump_no_protect_face_list); // 没有保护脸的人
        GX_END_FIELDS;
