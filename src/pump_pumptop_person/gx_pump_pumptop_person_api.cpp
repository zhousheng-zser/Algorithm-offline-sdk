...
namespace glasssix {
    ...
    public:
        ...
        algo_ptr() {
            try {
                ...
                nlohmann::json new_json(pump_pumptop_person_new_param{.device = _config->_pump_pumptop_person_config.device,
                    .models_directory            = _config->_configure_directory.models_directory
                    });
                char* new_result_c = parser_create_instance("g6.pump_pumptop_person.detect_code", new_json.dump().c_str());
    ...
    gx_pump_pumptop_person_api::gx_pump_pumptop_person_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
            ...
            try {
                auto result_pool = pool->enqueue([&] {
                    ...
                    for (int i = 0; i < person_list.size(); i++) {
                        if (person_list[i].score >= _config->_pump_pumptop_person_config.person_conf)
                    ...
                    nlohmann::json execute_json(pump_pumptop_person_detect_param{
                        .algo_params =
                            pump_pumptop_person_detect_param::optional_params{
                                .dyparams =
                                    pump_pumptop_person_detect_param::optional_params::dyparams_params{
                        ...
                        .data_params = pump_pumptop_person_detect_param::basic_params{
                    ...
                    ans = std::move(json::parse(*execute_result.result).get<pump_pumptop_person_detect_info_result>().detect_info);
                    return ans;
                });
                return result_pool.get();
            } catch (const std::exception& ex) {
                bool flag = write_dump_img(mat, "_pump_pumptop_person_dump.jpg", _config->_configure_directory.dump_img_directory);
    ...
    private:
        ...
        std::string empower_algorithm_version = share_platform_name + "_" + share_empower_language + "_pump_pumptop_person_V3.0.0";
    ...
    pump_pumptop_person_info gx_pump_pumptop_person_api::safe_production_pump_pumptop_person(const gx_img_api& mat) {
        ...
        return impl_->safe_production_pump_pumptop_person(mat, pedestrian_info_list.person_list);
    }
    //  泵业 泵顶无人检测
    pump_pumptop_person_info gx_pump_pumptop_person_api::safe_production_pump_pumptop_person(
        ...
        if (impl_ == nullptr)
            throw std::runtime_error{"You need to create a new object !\n"};
        return impl_->safe_production_pump_pumptop_person(mat, person_list);
    }
