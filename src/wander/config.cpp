...
namespace glasssix {
    ...
    void config::set_wander(const std::string& path) {
        std::ifstream lic_dir(path + "/wander.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: wander.json:directory =") + path + std::string("/wander.json"));

        json temp;
        std::ifstream(path + "/wander.json") >> temp;
        _wander_config = temp.get<wander_config>();
        wander_is_load = true;
    }
    void config::set_wander_limit(const std::string& path) {
        std::ifstream lic_dir(path + "/wander_limit.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(
                std::string("Error: wander_limit.json:directory =") + path + std::string("/wander_limit.json"));

        json temp;
        std::ifstream(path + "/wander_limit.json") >> temp;
        _wander_limit_config = temp.get<wander_limit_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "wander")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:wander:thread pool num is set to 1\n");
        }
    }
