...
namespace glasssix {
    ...
    void config::set_fighting(const std::string& path) {
        std::ifstream lic_dir(path + "/fighting.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: fighting.json:directory =") + path + std::string("/fighting.json"));

        json temp;
        std::ifstream(path + "/fighting.json") >> temp;
        _fighting_config = temp.get<fighting_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "fighting")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:fighting:thread pool num is set to 1\n");
        }
    }
