...
namespace glasssix {
    ...
    void config::set_workcloth(const std::string& path) {
        std::ifstream lic_dir(path + "/workcloth.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: workcloth.json:directory =") + path + std::string("/workcloth.json"));

        json temp;
        std::ifstream(path + "/workcloth.json") >> temp;
        _workcloth_config = temp.get<workcloth_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "workcloth")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:workcloth:thread pool num is set to 1\n");
        }
    }
