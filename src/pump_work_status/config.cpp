...
namespace glasssix {
    ...
    void config::set_pump_work_status(const std::string& path) {
        std::ifstream lic_dir(path + "/pump_work_status.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: pump_work_status.json:directory =") + path + std::string("/pump_work_status.json"));

        json temp;
        std::ifstream(path + "/pump_work_status.json") >> temp;
        _pump_work_status_config = temp.get<pump_work_status_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "pump_work_status")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:pump_work_status:thread pool num is set to 1\n");
        }
    }
