...
namespace glasssix {
    ...
    void config::set_smoke(const std::string& path) {
        std::ifstream lic_dir(path + "/smoke.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: smoke.json:directory =") + path + std::string("/smoke.json"));

        json temp;
        std::ifstream(path + "/smoke.json") >> temp;
        _smoke_config = temp.get<smoke_config>();
        smoke_is_load = true;
    }
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "smoke")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:smoke:thread pool num is set to 1\n");
        }
    }
