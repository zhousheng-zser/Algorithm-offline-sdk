...
namespace glasssix {
    ...
    void config::set_pumptop_helmet(const std::string& path) {
        std::ifstream lic_dir(path + "/pumptop_helmet.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: pumptop_helmet.json:directory =") + path + std::string("/pumptop_helmet.json"));

        json temp;
        std::ifstream(path + "/pumptop_helmet.json") >> temp;
        _pumptop_helmet_config = temp.get<pumptop_helmet_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "pumptop_helmet")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:pumptop_helmet:thread pool num is set to 1\n");
        }
    }
