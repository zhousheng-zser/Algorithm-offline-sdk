...
namespace glasssix {
    ...
    void config::set_pump_vesthelmet(const std::string& path) {
        std::ifstream lic_dir(path + "/pump_vesthelmet.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: pump_vesthelmet.json:directory =") + path + std::string("/pump_vesthelmet.json"));

        json temp;
        std::ifstream(path + "/pump_vesthelmet.json") >> temp;
        _pump_vesthelmet_config = temp.get<pump_vesthelmet_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "pump_vesthelmet")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:pump_vesthelmet:thread pool num is set to 1\n");
        }
    }
