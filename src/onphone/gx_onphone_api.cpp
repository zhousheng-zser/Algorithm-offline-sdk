...
namespace glasssix {
    ...
    public:
        ...
        algo_ptr() {
            try {
                ...
                nlohmann::json new_json(onphone_new_param{.device = _config->_onphone_config.device,
                    .models_directory                             = _config->_configure_directory.models_directory});
                char* new_result_c = parser_create_instance("g6.onphone.detect_code", new_json.dump().c_str());
    ...
    gx_onphone_api::gx_onphone_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
        ...
        onphone_info safe_production_onphone(const gx_img_api& mat, const std::vector<head_info>& head_info_list) {
            try {
                auto result_pool = pool->enqueue([&] {
                    ...
                    onphone_info ans;
                    std::vector<head_info> head_list_temp;
...
#endif
                    nlohmann::json execute_json(onphone_detect_param{
                        .algo_params =
                            onphone_detect_param::optional_params{
                                .dyparams =
                                    onphone_detect_param::optional_params::dyparams_params{.posture_info_list = nullptr,
                    ...
                    ans = std::move(json::parse(*execute_result.result).get<onphone_detect_info_result>().detect_info);
                    return ans;
                });
                return result_pool.get();
            } catch (const std::exception& ex) {
                bool flag = write_dump_img(mat, "_onphone_dump.jpg", _config->_configure_directory.dump_img_directory);
            ...
            try {
                auto result_pool = pool->enqueue([&] {
                    ...
                    for (int i = 0; i < posture_info_list.size(); i++) {
                        if (posture_info_list[i].score >= _config->_onphone_config.posture_conf_thres)
                    ...
                    nlohmann::json execute_json(onphone_detect_param{
                        .algo_params =
                            onphone_detect_param::optional_params{
                                .dyparams =
                                    onphone_detect_param::optional_params::dyparams_params{
                                        .posture_info_list =
                                            std::make_unique<std::vector<posture_info>>(posture_list_temp),
    ...
    private:
        ...
        std::string empower_algorithm_version = share_platform_name + "_" + share_empower_language + "_onphone_V3.0.0";
    ...
    onphone_info gx_onphone_api::safe_production_onphone(
        const gx_img_api& mat, const std::vector<head_info>& head_info_list) {
        if (impl_ == nullptr)
            throw std::runtime_error{"You need to create a new object !\n"};
        return impl_->safe_production_onphone(mat, head_info_list);
    }
    //  安全生产 打电话检测 依赖姿态
    onphone_info gx_onphone_api::safe_production_onphone(
        const gx_img_api& mat, const std::vector<posture_info>& posture_info_list) {
        if (impl_ == nullptr)
            throw std::runtime_error{"You need to create a new object !\n"};
        return impl_->safe_production_onphone(mat, posture_info_list);
    }
