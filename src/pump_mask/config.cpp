...
namespace glasssix {
    ...
    void config::set_pump_mask(const std::string& path) {
        std::ifstream lic_dir(path + "/pump_mask.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: pump_mask.json:directory =") + path + std::string("/pump_mask.json"));

        json temp;
        std::ifstream(path + "/pump_mask.json") >> temp;
        _pump_mask_config = temp.get<pump_mask_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "pump_mask")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:pump_mask:thread pool num is set to 1\n");
        }
    }
