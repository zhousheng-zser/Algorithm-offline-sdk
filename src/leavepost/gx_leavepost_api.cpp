...
namespace glasssix {
    ...
    public:
        ...
        algo_ptr() {
            try {
                ...
                nlohmann::json new_json(leavepost_new_param{.device = _config->_leavepost_config.device,
                    .models_directory            = _config->_configure_directory.models_directory});
                char* new_result_c = parser_create_instance("g6.leavepost.detect_code", new_json.dump().c_str());
    ...
    gx_leavepost_api::gx_leavepost_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
        ...
        leavepost_info safe_production_leavepost(const gx_img_api& mat) {
            try {
                auto result_pool = pool->enqueue([&] {
                    ...
                    nlohmann::json execute_json(leavepost_detect_param{
                        .algo_params =
                            leavepost_detect_param::optional_params{.dyparams =
                                                                   leavepost_detect_param::optional_params::dyparams_params{
                                                                       .conf_thres = _config->_leavepost_config.conf_thres,
                                                                       .nms_thres  = _config->_leavepost_config.nms_thres}},
                        .data_params =
                            leavepost_detect_param::basic_params{.height = mat.get_rows(), .width = mat.get_cols()}
                    ...
                    ans = std::move(json::parse(*execute_result.result).get<leavepost_detect_info_result>().detect_info);
                    return ans;
                });
                return result_pool.get();
            } catch (const std::exception& ex) {
                bool flag = write_dump_img(mat, "_leavepost_dump.jpg", _config->_configure_directory.dump_img_directory);
    ...
    private:
        ...
        std::string empower_algorithm_version = share_platform_name + "_" + share_empower_language + "_leavepost_V3.0.0";
    ...
    //  安全生产 烟雾检测
    leavepost_info gx_leavepost_api::safe_production_leavepost(const gx_img_api& mat) {
        if (impl_ == nullptr)
            throw std::runtime_error{"You need to create a new object !\n"};
        return impl_->safe_production_leavepost(mat);
    }
