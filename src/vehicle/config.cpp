...
namespace glasssix {
    ...
    void config::set_vehicle(const std::string& path) {
        std::ifstream lic_dir(path + "/vehicle.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: vehicle.json:directory =") + path + std::string("/vehicle.json"));

        json temp;
        std::ifstream(path + "/vehicle.json") >> temp;
        _vehicle_config = temp.get<vehicle_config>();
    ...
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "vehicle")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:vehicle:thread pool num is set to 1\n");
        }
    }
