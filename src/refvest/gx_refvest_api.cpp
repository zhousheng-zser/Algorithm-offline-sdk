...
namespace glasssix {
    ...
    public:
        ...
        algo_ptr() {
            try {
                ...
                nlohmann::json new_json(refvest_new_param{.device = _config->_refvest_config.device,
                    .models_directory            = _config->_configure_directory.models_directory
                    });
                char* new_result_c = parser_create_instance("g6.refvest.detect_code", new_json.dump().c_str());
    ...
    gx_refvest_api::gx_refvest_api(const std::string& config_path) : impl_{std::make_unique<impl>(config_path)} {}
        ...
        refvest_info safe_production_refvest(
            ...
            try {
                auto result_pool = pool->enqueue([&] {
                    ...
                    nlohmann::json execute_json(refvest_detect_param{
                        .algo_params =
                            refvest_detect_param::optional_params{
                                .dyparams =
                                    refvest_detect_param::optional_params::dyparams_params{
                        ...
                        .data_params =
                            refvest_detect_param::basic_params{.height = mat.get_rows(), .width = mat.get_cols()}
                    });
                    ...
                    ans = std::move(json::parse(execute_result.result).get<refvest_detect_info_result>().detect_info);
                    return ans;
                });
                return result_pool.get();
            } catch (const std::exception& ex) {
                bool flag = write_dump_img(mat, "_refvest_dump.jpg", _config->_configure_directory.dump_img_directory);
    ...
    private:
        ...
        std::string empower_algorithm_version = share_platform_name + "_" + share_empower_language + "_refvest_V3.0.0";
