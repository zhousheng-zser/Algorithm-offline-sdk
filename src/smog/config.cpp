...
namespace glasssix {
    ...
    void config::set_smog(const std::string& path) {
        std::ifstream lic_dir(path + "/smog.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: smog.json:directory =") + path + std::string("/smog.json"));

        json temp;
        std::ifstream(path + "/smog.json") >> temp;
        _smog_config = temp.get<smog_config>();
        smog_is_load = true;
    }
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "smog")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:smog:thread pool num is set to 1\n");
        }
    }
