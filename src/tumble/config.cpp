...
namespace glasssix {
    ...
    void config::set_tumble(const std::string& path) {
        std::ifstream lic_dir(path + "/tumble.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: tumble.json:directory =") + path + std::string("/tumble.json"));

        json temp;
        std::ifstream(path + "/tumble.json") >> temp;
        _tumble_config = temp.get<tumble_config>();
        tumble_is_load = true;
    }
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "tumble")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:tumble:thread pool num is set to 1\n");
        }
    }
