...
namespace glasssix {
    ...
    void config::set_crowd(const std::string& path) {
        std::ifstream lic_dir(path + "/crowd.json");
        if (!lic_dir.is_open())
            throw std::runtime_error(std::string("Error: crowd.json:directory =") + path + std::string("/crowd.json"));

        json temp;
        std::ifstream(path + "/crowd.json") >> temp;
        _crowd_config = temp.get<crowd_config>();
        crowd_is_load = true;
    }
    void config::set_thread_pool_num(const std::string& path) {
        ...
        try {
            ...
            for (thread_pool_num::algo& val : list_.algo_list) {
                if (val.protocols == "crowd")
                    thread_pool_val = val.num;
            }
        } catch (...) {
            printf("Warning:crowd:thread pool num is set to 1\n");
        }
    }
